-- Do not infer regulatory scope from free-form category text.
-- Bulk imports must declare a supported sector and fail closed when no active
-- requirement catalogue exists for that sector.

alter table public.products drop constraint if exists products_sector_check;
alter table public.products add constraint products_sector_check
  check (sector in ('construction','consumer'));
  ...
  for row_data in select value from jsonb_array_elements(p_rows) loop
    ...
    begin
      ...
      if requirement_count = 0 then
        raise exception 'Aucun référentiel réglementaire actif pour le secteur % ; ligne bloquée', sector_value;
      end if;
