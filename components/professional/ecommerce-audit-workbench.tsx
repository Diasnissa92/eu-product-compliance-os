...
const emptyListing: EcommerceListingInput = {
  title: "", productIdentifier: "", manufacturerName: "", manufacturerPostalAddress: "", manufacturerElectronicAddress: "",
  responsiblePersonName: "", responsiblePersonPostalAddress: "", responsiblePersonElectronicAddress: "", warnings: "", language: "Français", traceabilityImage: false,
};
...
export function EcommerceAuditWorkbench({ products, audits: initialAudits, persistence }: { products: ProfessionalProduct[]; audits: EcommerceAuditRecord[]; persistence?: Persistence }) {
  ...
  return <div className="professional-stack">
    <section className="panel professional-form-panel">
      <div className="professional-panel-heading"><span className="feature-icon feature-icon-blue"><ScanSearch size={21} /></span><div><span className="eyebrow">Contrôle GPSR</span><h2>Vérifier une offre avant sa publication</h2><p>Renseignez ce qui est réellement visible pour l’acheteur. L’outil détecte les mentions absentes, sans aspirer ni stocker la page marchande.</p></div></div>
      <form className="professional-form form-grid" onSubmit={runAudit}>
        ...
        <label className="field field-full"><span>URL de l’offre <em>optionnel, conservé comme preuve</em></span><input type="url" value={listingUrl} onChange={(event) => setListingUrl(event.target.value)} placeholder="https://…" /></label>
        ...
        <label className="field"><span>Titre du produit</span><input value={listing.title} onChange={(event) => update("title", event.target.value)} /></label>
        <label className="field"><span>Référence / type / lot</span><input value={listing.productIdentifier} onChange={(event) => update("productIdentifier", event.target.value)} /></label>
        <label className="field"><span>Nom du fabricant</span><input value={listing.manufacturerName} onChange={(event) => update("manufacturerName", event.target.value)} /></label>
        <label className="field"><span>Adresse postale du fabricant</span><input value={listing.manufacturerPostalAddress} onChange={(event) => update("manufacturerPostalAddress", event.target.value)} /></label>
        <label className="field field-full"><span>Adresse électronique du fabricant</span><input value={listing.manufacturerElectronicAddress} onChange={(event) => update("manufacturerElectronicAddress", event.target.value)} placeholder="contact@fabricant.eu ou formulaire clairement accessible" /></label>
        {!manufacturerInEu ? <><label className="field"><span>Nom du responsable UE</span><input value={listing.responsiblePersonName} onChange={(event) => update("responsiblePersonName", event.target.value)} /></label><label className="field"><span>Adresse postale du responsable UE</span><input value={listing.responsiblePersonPostalAddress} onChange={(event) => update("responsiblePersonPostalAddress", event.target.value)} /></label><label className="field field-full"><span>Adresse électronique du responsable UE</span><input value={listing.responsiblePersonElectronicAddress} onChange={(event) => update("responsiblePersonElectronicAddress", event.target.value)} /></label></> : null}
        <label className="field field-full"><span>Avertissements et informations de sécurité visibles</span><textarea rows={4} value={listing.warnings} onChange={(event) => update("warnings", event.target.value)} placeholder="Copiez ici les avertissements affichés avant l’achat." /></label>
        <label className="field"><span>Langue principale</span><input value={listing.language} onChange={(event) => update("language", event.target.value)} /></label>
        <label className="check-card"><input type="checkbox" checked={listing.traceabilityImage} onChange={(event) => update("traceabilityImage", event.target.checked)} /><span><strong>Image de traçabilité présente</strong><small>Produit et identification reconnaissables.</small></span></label>
