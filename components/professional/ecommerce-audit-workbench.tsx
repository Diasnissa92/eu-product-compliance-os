...
const emptyListing: EcommerceListingInput = {
  ...
  responsiblePersonName: "", responsiblePersonPostalAddress: "", responsiblePersonElectronicAddress: "", warningsApplicable: true,
  ...
  return <div className="professional-stack">
    <section className="panel professional-form-panel">
      ...
      <form className="professional-form form-grid" onSubmit={runAudit}>
        ...
        <label className="check-card field-full"><input type="checkbox" checked={listing.warningsApplicable} onChange={(event) => update("warningsApplicable", event.target.checked)} /><span><strong>Des avertissements ou informations de sécurité sont applicables</strong><small>Décochez uniquement si l’analyse de risques conclut et justifie qu’aucune mention spécifique n’est applicable.</small></span></label>
        {listing.warningsApplicable ? <label className="field field-full"><span>Avertissements et informations de sécurité visibles</span><textarea rows={4} value={listing.warnings} onChange={(event) => update("warnings", event.target.value)} placeholder="Copiez ici les avertissements affichés avant l’achat." /></label> : <label className="field field-full"><span>Justification de non-applicabilité</span><textarea rows={3} value={listing.warningsNotApplicableReason} onChange={(event) => update("warningsNotApplicableReason", event.target.value)} placeholder="Référencez l’analyse de risques ou la conclusion validée." /></label>}
