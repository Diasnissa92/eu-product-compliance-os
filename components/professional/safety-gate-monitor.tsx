...
export function SafetyGateMonitor({ products, watches: initialWatches, matches: initialMatches, persistence }: { products: ProfessionalProduct[]; watches: SafetyGateWatchRecord[]; matches: SafetyGateMatchRecord[]; persistence?: Persistence }) {
  ...
  async function checkWatch(watch: SafetyGateWatchRecord) {
    ...
    setFeedback("Recherche manuelle ouverte sur Safety Gate. La date de dernière synchronisation automatique reste inchangée afin de conserver une traçabilité exacte.");
  ...
  async function addMatch(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (!alertReference || alertReference.length > 80 || !title || title.length > 500 || !officialAlertPattern.test(alertUrl)) throw new Error("Ajoutez une référence, un titre et l’adresse exacte de la page d’alerte officielle.");
      if (!['serious', 'high', 'medium'].includes(riskLevel)) throw new Error("Niveau de risque invalide.");
  ...
  async function createIncident(match: SafetyGateMatchRecord) {
    ...
    try {
      ...
      setFeedback("Incident ouvert ou rattaché à l’incident existant. Retrouvez-le dans le registre Incidents et rappels.");
      router.refresh();
