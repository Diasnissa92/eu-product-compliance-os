...
import { AlertTriangle, Clipboard, ExternalLink, Eye, Plus, Radar, ShieldAlert } from "lucide-react";
...
export function SafetyGateMonitor({ products, watches: initialWatches, matches: initialMatches, persistence }: { products: ProfessionalProduct[]; watches: SafetyGateWatchRecord[]; matches: SafetyGateMatchRecord[]; persistence?: Persistence }) {
  ...
  async function addWatch(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setFeedback("Veille enregistrée. Utilisez le contrôle officiel pour rechercher les alertes correspondantes.");
      router.refresh();
  ...
  async function addMatch(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (!alertReference || !title || !alertUrl.startsWith("https://ec.europa.eu/")) throw new Error("Ajoutez une référence, un titre et le lien officiel ec.europa.eu de l’alerte.");
  ...
  async function createIncident(match: SafetyGateMatchRecord) {
    ...
    try {
      if (persistence) {
        ...
        const { error: incidentError } = await supabase.from("product_incidents").insert({ org_id: persistence.organizationId, product_id: match.productId, created_by: persistence.userId, source: "safety_gate", title: match.title, reference: match.alertReference, severity: match.riskLevel === "serious" ? "critical" : "high", status: "investigating", description: `Alerte Safety Gate à évaluer : ${match.alertUrl}`, recall_required: false });
        if (incidentError) throw incidentError;
        const { error: updateError } = await supabase.from("safety_gate_matches").update({ status: "incident_created", updated_at: new Date().toISOString() }).eq("id", match.id).eq("org_id", persistence.organizationId);
  ...
  return <div className="professional-stack">
    <section className="professional-summary-grid"><article><span>Veilles actives</span><strong>{watches.filter((item) => item.enabled).length}</strong><small>Marques et références</small></article><article><span>Correspondances</span><strong>{matches.filter((item) => item.status === "new").length}</strong><small>À qualifier</small></article><article><span>Incidents ouverts</span><strong>{matches.filter((item) => item.status === "incident_created").length}</strong><small>Créés depuis Safety Gate</small></article></section>
    <section className="panel professional-form-panel"><div className="professional-panel-heading"><span className="feature-icon feature-icon-amber"><Radar size={21} /></span><div><span className="eyebrow">Surveillance officielle</span><h2>Créer une veille ciblée</h2><p>Associez des marques, références et catégories à un produit, puis consignez les correspondances du portail officiel.</p></div></div><form className="professional-form form-grid" onSubmit={addWatch}><label className="field"><span>Produit <em>optionnel</em></span><select name="productId" defaultValue=""><option value="">Veille portefeuille</option>{products.map((product) => <option value={product.id} key={product.id}>{product.name} · {product.sku}</option>)}</select></label><label className="field"><span>Nom de la veille</span><input name="label" required placeholder="Ex. Marque BELLARO" /></label><label className="field"><span>Catégorie Safety Gate</span><input name="category" placeholder="Ex. Appareils électriques" /></label><label className="field"><span>Mots-clés</span><input name="keywords" required placeholder="BELLARO, référence, modèle" /></label><div className="professional-form-actions field-full"><a className="inline-link" href={officialSafetyGateUrl} target="_blank" rel="noreferrer"><ExternalLink size={15} />Portail officiel Safety Gate</a><button className="button button-primary" disabled={saving}><Plus size={17} />Ajouter la veille</button></div></form></section>
    <section className="panel professional-list-panel"><div className="professional-panel-heading compact"><div><span className="eyebrow">Recherches enregistrées</span><h2>Veilles actives</h2></div><span className="professional-count">{watches.length}</span></div>{watches.length ? <div className="professional-list">{watches.map((watch) => <article className="professional-row" key={watch.id}><span className="feature-icon feature-icon-amber"><Eye size={18} /></span><div className="professional-row-copy"><strong>{watch.label}</strong><p>{watch.keywords.join(" · ")}</p><small>{watch.lastCheckedAt ? `Contrôlée le ${new Intl.DateTimeFormat("fr-FR").format(new Date(watch.lastCheckedAt))}` : "Premier contrôle à effectuer"}</small></div><button className="button button-secondary button-small" type="button" onClick={() => checkWatch(watch)}><Clipboard size={15} />Contrôler</button></article>)}</div> : <div className="empty-state"><Radar size={29} /><strong>Aucune veille</strong><p>Ajoutez des mots-clés pour structurer vos contrôles Safety Gate.</p></div>}</section>
    <section className="panel professional-form-panel"><div className="professional-panel-heading"><span className="feature-icon feature-icon-red"><ShieldAlert size={21} /></span><div><span className="eyebrow">Résultat du contrôle</span><h2>Consigner une correspondance</h2><p>Enregistrez uniquement une alerte provenant du domaine officiel de la Commission européenne.</p></div></div><form className="professional-form form-grid" onSubmit={addMatch}><label className="field"><span>Veille associée</span><select name="watchId" defaultValue=""><option value="">Sans veille</option>{watches.map((watch) => <option value={watch.id} key={watch.id}>{watch.label}</option>)}</select></label><label className="field"><span>Niveau de risque</span><select name="riskLevel" defaultValue="serious"><option value="serious">Risque sérieux</option><option value="high">Élevé</option><option value="medium">Modéré</option></select></label><label className="field"><span>Référence de l’alerte</span><input name="alertReference" required placeholder="Ex. SR/01234/26" /></label><label className="field"><span>Titre / produit concerné</span><input name="title" required /></label><label className="field field-full"><span>Lien officiel</span><input name="alertUrl" type="url" required placeholder="https://ec.europa.eu/safety-gate-alerts/…" /></label><div className="professional-form-actions field-full"><span /><button className="button button-primary" disabled={saving}><AlertTriangle size={17} />Enregistrer l’alerte</button></div>{feedback ? <p className="form-feedback form-feedback-success field-full" role="status">{feedback}</p> : null}{error ? <p className="form-feedback form-feedback-error field-full" role="alert">{error}</p> : null}</form></section>
