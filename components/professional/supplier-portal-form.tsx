...
export function SupplierPortalForm({ token, supplierName, supplierEmail }: { token: string; supplierName: string; supplierEmail: string }) {
  ...
  if (complete) return <div className="portal-success"><CheckCircle2 size={38} /><h2>Document transmis</h2><p>L’équipe conformité a reçu votre réponse. Vous pouvez fermer cette page.</p></div>;
  ...
  return <form className="portal-form form-grid" onSubmit={submit}>
    ...
    <label className="field"><span>Votre e-mail</span><input name="supplierEmail" type="email" defaultValue={supplierEmail} required /></label>
    ...
    <div className="professional-form-actions field-full"><span className="secure-note"><ShieldCheck size={16} />Réponse liée uniquement à cette demande</span><button className="button button-primary" disabled={saving}><Send size={17} />{saving ? "Envoi…" : "Transmettre la réponse"}</button></div>
